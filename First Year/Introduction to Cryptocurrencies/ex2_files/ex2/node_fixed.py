    ...
    def validate_block(self, sender: 'Node', block: Block) -> bool:
        ...
        for tx in block.get_transactions():
            ...
            if tx.input is None:
                ...
                input_tx = find_transaction(sender, tx.input)
...
def find_transaction(self:Node, txid: TxID) -> List[Transaction]:
    found_transactions: List[Transaction] = list()
    curr_hash = self.get_latest_hash()
    while curr_hash != GENESIS_BLOCK_PREV:
        curr_block = self.get_block(curr_hash)
        tx = curr_block.find_transaction(txid)
        if tx:
            found_transactions.append(tx)
