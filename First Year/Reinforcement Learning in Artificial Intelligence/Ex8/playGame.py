    ...
    while True:
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                x, y = mx // game.title_size, my // game.title_size
...
def draw_board(game:Breakthrough, screen):
    for y in range(game.board_size):
        for x in range(game.board_size):
            ...
            pygame.draw.rect(screen, color, (x * game.title_size, y * game.title_size, game.title_size, game.title_size))
            piece = game.board[y][x]
            if piece != 0:
                piece_color = Colors.White if piece == 1 else Colors.Black
                pygame.draw.circle(screen, piece_color, (x * game.title_size + game.title_size // 2, y * game.title_size + game.title_size // 2), game.title_size // 3)
                if piece == 1:  # Add black border for white pieces
                    pygame.draw.circle(screen, Colors.Black, (x * game.title_size + game.title_size // 2, y * game.title_size + game.title_size // 2), game.title_size // 3, 2)

def draw_selection(game, screen):
    if game.selection:
        x, y = game.selection
        pygame.draw.rect(screen, Colors.LightGray, (x * game.title_size, y * game.title_size, game.title_size, game.title_size), 3)
        for move in game.valid_moves((x, y)):
            mx, my = move
            pygame.draw.rect(screen, Colors.Green, (mx * game.title_size, my * game.title_size, game.title_size, game.title_size), 3)
